...
func (seg *Segmenter) segmentWordsWithValue(text []Text, searchMode bool) []Segment {
	// 搜索模式下该分词已无继续划分可能的情况
	if searchMode && len(text) == 1 {
		return []Segment{}
	}

	maliciousValue := 0 // 加入一个恶意值
	...
	for index := len(text) - 1; index >= 0; {
		...
		numSeg++
		index = location - 1
		maliciousValue += jumpers[index].token.frequency // 将频率做成恶意值来使用
	}
